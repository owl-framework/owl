...
PHP_METHOD(Owl_Application, dispatch) {
	...
	zephir_array_fetch_string(&_2, parameters, SL("controller"), PH_NOISY | PH_READONLY, "owl/Application.zep", 67 TSRMLS_CC);
	...
	/* try_start_1: */
		...
		if (!(zephir_class_exists(handlerClass, 1 TSRMLS_CC))) {
			...
			zephir_throw_exception_debug(_4, "owl/Application.zep", 78 TSRMLS_CC);
			goto try_end_1;
		...
		zephir_check_temp_parameter(_4);
		zephir_check_call_status_or_jump(try_end_1);
		zephir_array_fetch_string(&_11, parameters, SL("action"), PH_NOISY | PH_READONLY, "owl/Application.zep", 85 TSRMLS_CC);
...
PHP_METHOD(Owl_Application, handle) {
	...
	int ZEPHIR_LAST_CALL_STATUS;
	zval *request, *response = NULL, *matchedRoute = NULL, *router = NULL, *_0, *_1 = NULL, *_2, *_3, *parameters = NULL, *_4, *_7;
	...
	if (zephir_is_true(matchedRoute)) {
		...
		ZEPHIR_CALL_METHOD(NULL, this_ptr, "dispatch", &_5, parameters, matchedRoute, response);
		zephir_check_call_status();
	} else {
		ZEPHIR_INIT_NVAR(_1);
		ZVAL_LONG(_1, 404);
		ZEPHIR_CALL_METHOD(NULL, response, "setcode", &_6, _1);
		zephir_check_call_status();
		ZEPHIR_CALL_METHOD(NULL, this_ptr, "dispatch", &_5, parameters, matchedRoute, response);
		zephir_check_call_status();
	}
	_7 = zephir_fetch_nproperty_this(this_ptr, SL("eventManager"), PH_NOISY_CC);
	ZEPHIR_INIT_NVAR(_1);
	ZVAL_STRING(_1, "app:afterHandle", ZEPHIR_TEMP_PARAM_COPY);
	ZEPHIR_CALL_METHOD(NULL, _7, "emit", NULL, _1, this_ptr);
	zephir_check_temp_parameter(_1);
	zephir_check_call_status();
