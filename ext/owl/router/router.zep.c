...
PHP_METHOD(Owl_Router_Router, add) {
	...
	if (_2) {
		...
		ZEPHIR_CALL_METHOD(NULL, route, "__construct", NULL, 32, uri);
		zephir_check_call_status();
	} else {
		object_init_ex(route, owl_router_http_dynamicroute_ce);
		ZEPHIR_CALL_METHOD(NULL, route, "__construct", NULL, 33, uri);
		zephir_check_call_status();
	}
...
PHP_METHOD(Owl_Router_Router, matchRequest) {
	...
	zephir_check_call_status();
	ZEPHIR_CALL_FUNCTION(&result, "parse_url", NULL, 20, _0);
	zephir_check_call_status();
	ZEPHIR_OBS_VAR(path);
