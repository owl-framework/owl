...
PHP_METHOD(Owl_DBAL_Connection, __construct) {
	...
	if (zephir_array_isset_string(parameters, SS("driver"))) {
		zephir_array_fetch_string(&_0, parameters, SL("driver"), PH_NOISY | PH_READONLY, "owl/DBAL/Connection.zep", 39 TSRMLS_CC);
		...
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(owl_dbal_exception_ce, "Driver not found in $parameters", "owl/DBAL/Connection.zep", 46);
		return;
	}
	_1 = zephir_fetch_nproperty_this(this_ptr, SL("driver"), PH_NOISY_CC);
...
PHP_METHOD(Owl_DBAL_Connection, insert) {
	...
	zephir_is_iterable(data, &_1, &_0, 1, 0, "owl/DBAL/Connection.zep", 75);
	for (
	  ...
	  ; zephir_hash_move_forward_ex(_1, &_0)
		...
		if (zephir_array_isset(types, columnName)) {
			zephir_array_fetch(&_3, types, columnName, PH_NOISY | PH_READONLY, "owl/DBAL/Connection.zep", 64 TSRMLS_CC);
			if (ZEPHIR_IS_LONG(_3, 10)) {
				zephir_array_append(&set, value, PH_SEPARATE, "owl/DBAL/Connection.zep", 65);
				zephir_array_unset(&data, columnName, PH_SEPARATE);
			} else {
				ZEPHIR_INIT_LNVAR(_4);
				ZEPHIR_CONCAT_SVS(_4, ":", columnName, "");
				zephir_array_append(&set, _4, PH_SEPARATE, "owl/DBAL/Connection.zep", 68);
			}
		} else {
			ZEPHIR_INIT_LNVAR(_4);
			ZEPHIR_CONCAT_SVS(_4, ":", columnName, "");
			zephir_array_append(&set, _4, PH_SEPARATE, "owl/DBAL/Connection.zep", 71);
		}
	}
...
PHP_METHOD(Owl_DBAL_Connection, update) {
	...
	zephir_is_iterable(_1, &_3, &_2, 0, 0, "owl/DBAL/Connection.zep", 93);
	for (
	  ...
	  ; zephir_hash_move_forward_ex(_3, &_2)
		...
		zephir_array_append(&set, _7, PH_SEPARATE, "owl/DBAL/Connection.zep", 90);
	}
	_5 = zephir_fetch_nproperty_this(this_ptr, SL("platform"), PH_NOISY_CC);
	ZEPHIR_CALL_METHOD(&_6, _5, "wrap", NULL, 0, table);
	zephir_check_call_status();
	...
	if (ZEPHIR_GT_LONG(cnt, 0)) {
		...
		if (ZEPHIR_IS_LONG(cnt, 1)) {
			...
			ZEPHIR_CALL_FUNCTION(&_11, "key", NULL, 4, identifiers);
			...
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(owl_dbal_exception_ce, "It's not implemented", "owl/DBAL/Connection.zep", 104);
			return;
		}
	}
	...
	zephir_check_call_status();
	ZEPHIR_INIT_VAR(_13);
	ZEPHIR_CALL_FUNCTION(&_6, "array_values", &_14, 5, data);
	zephir_check_call_status();
	ZEPHIR_CALL_FUNCTION(&_10, "array_values", &_14, 5, identifiers);
	zephir_check_call_status();
...
PHP_METHOD(Owl_DBAL_Connection, delete) {
	...
	zephir_is_iterable(identifiers, &_3, &_2, 0, 0, "owl/DBAL/Connection.zep", 149);
	for (
	  ; zephir_hash_get_current_data_ex(_3, (void**) &_4, &_2) == SUCCESS
	  ; zephir_hash_move_forward_ex(_3, &_2)
	) {
