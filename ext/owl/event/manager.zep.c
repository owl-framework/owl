		...
		for (
		  ...
		  ; zephir_hash_move_forward_ex(_2, &_1)
			...
			if (Z_TYPE_P(callback) == IS_ARRAY) {
				...
				ZEPHIR_CALL_METHOD(NULL, _6, "__construct", &_7, 16, eventName, data);
				...
				ZEPHIR_CALL_METHOD(NULL, _4, "__construct", &_7, 16, eventName, data);
				zephir_check_call_status();
...
static zend_object_value zephir_init_properties_Owl_Event_Manager(zend_class_entry *class_type TSRMLS_DC) {
	...
	
	{
		...
		if (Z_TYPE_P(_0) == IS_NULL) {
			ZEPHIR_INIT_VAR(_1);
			array_init(_1);
			zephir_update_property_this(this_ptr, SL("listeners"), _1 TSRMLS_CC);
		}
		ZEPHIR_MM_RESTORE();
